...
public class PatientCareController {
    ...
    //Returns a list of vaccines by Dose
    @GetMapping("/filter")
    public ResponseEntity<List<PatientCareDTO>> getPCsByVaccineIdAndDoseAndDoseDate(
            @RequestParam(name = "vaccine", required = false) Integer id,
            @RequestParam(name = "dose", required = false) Byte dose,
            @RequestParam(name = "doseDate", required = false) String doseDate) {
        LocalDate parsedDoseDate = null;
        if (doseDate != null) {
            DateTimeFormatter df = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            parsedDoseDate = LocalDate.parse(doseDate, df);
        }        
        List<PatientCareDTO> _patientCareListDTO = patientCareService.getPCsByVaccineIdAndDoseAndDoseDate(id, dose, parsedDoseDate);
        return new ResponseEntity<>(_patientCareListDTO, HttpStatus.OK);
    }
